...
Tests (`tests/`, Vitest, no DOM) cover the parts where a bug would be silent: every deck builds, selector resolution, and flow semantics. `tests/world.ts` is a `FlowWorld` with a virtual clock that records events, so flows run headlessly and instantly. When adding a test, check it can fail: break the behaviour on purpose and watch it go red.

Rendering has no tests; check it visually: run the dev server, open `http://localhost:<port>/?deck=<id>#<scene>`, and screenshot with the Playwright MCP tools. Without `?deck=` you get the deck picker. Hash changes switch scenes without reloading; add a throwaway parameter (`?deck=ota&r=1#9`) to force a fresh load. Switching deck is always a page load, so the engine never has to tear itself down. Viewer controls: ← → / Space / click empty space to step, click a shape or tower slab for its info box (Escape closes it), pinch or ctrl+scroll to zoom, scroll or drag to pan, option+drag to orbit, two-finger rotate to turn (Safari only), `v` or the 2D/3D switch to flip between top-down and the scene's own angle, `0` to reset the view. The nav at the bottom right holds the deck link, that switch, reset and the scene markers; the top left has only the deck title. The user tests in **Safari**. Trackpad pinch and rotate arrive there as `gesture*` events, not ctrl+wheel; both paths are handled in `controls.ts`. Chrome on macOS has no rotate gesture.
...
| `engine/flow.ts` | Flows, with no rendering: tokens, paths, gathers and which instances a hop reaches. Runs against a `FlowWorld` and emits `hop`/`arrive`/`drop` events. Tested directly. |
| `engine/traffic.ts` | All packets, and the `FlowWorld` flows run against: turns their events into packets on two instanced `PacketLayer`s (logical, and runtime underneath). `StreamRunner` handles background traffic. |
| `engine/random.ts` | Seeded generator. `?seed=<n>` makes a deck's traffic repeat exactly; decks use it to generate stable content. |
...
- Flows never touch three.js: they ask the world for `travelTime`, `cluster` and `lowered`, and emit events. Keep it that way, or the tests stop working.
...
- **Flat by default, 3D for runtime moments.** Scenes with `runtime` default to tilt 62 and turn −38. The 2D/3D switch (`Director.setFlat`) overrides a scene's angles until the scene changes, so any scene can be seen either way.
- **Scrims** behind the caption, nav and title are background-coloured gradients, invisible over empty space. The dot grid is the only thing the scene's fog applies to (everything else sets `fog: false`), so it fades into the distance in tilted views instead of aliasing. The logical diagram sits on a frosted sheet; `fog` sets how much it hides the towers beneath.
