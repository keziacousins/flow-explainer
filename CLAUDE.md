...
There are no tests. Check changes visually: run the dev server, open `http://localhost:<port>/#<scene>`, and screenshot with the Playwright MCP tools. Hash changes switch scenes without reloading; add a throwaway query string (`?r=1#9`) to force a fresh load. Viewer controls: ← → / Space / click to step, pinch or ctrl+scroll to zoom, scroll or drag to pan, option+drag to orbit, `0` to reset the view. The user tests in **Safari**. Trackpad pinch there arrives as `gesture*` events, not ctrl+wheel, and both paths are handled in `controls.ts`.
...
| `engine/callouts.ts` | HTML labels with SVG elbowed leader lines, placed each frame in empty screen space. |
